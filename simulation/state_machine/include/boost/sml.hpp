...
struct unique_impl<type<T1, Rs...>, T2, Ts...>
  : conditional_t<is_base_of<type<T2>, T1>::value,
...
struct is_unique<T1, T2, Ts...>
  : conditional_t<is_base_of<type<T2>, T1>::value, false_type,
                  is_unique<inherit<T1, type<T2> >, Ts...> >
{
};
template <class... Ts>
...
struct tuple_type
{
  explicit tuple_type(const T& object)
    : value(object)
  {
  }
  T value;
...
T&
get_by_id(tuple_type<N, T>* object)
{
...
struct pool_type
{
  explicit pool_type(const T& object)
    : value(object)
  {
  }
  template <class TObject>
  pool_type(const init& i, const TObject& object)
    : value(i, object)
  {
  }
  T value;
...
struct pool : pool_type<Ts>...
{
  ...
  pool(init&&, pool<TArgs...>&& p)
    : pool_type<Ts>(try_get<Ts>(&p))...
  {
  }
  template <class... TArgs>
  pool(const pool<TArgs...>& p)
    : pool_type<Ts>(init{}, p)...
  {
  }
};
...
constexpr int
max_impl(int r, int i)
{
  return r > i ? r : i;
}
constexpr int
max_impl(int r, int i, int ints...)
{
...
{
  explicit zero_wrapper(const TExpr& expr)
    : TExpr(expr)
  {
  }
  const TExpr& get() const
  {
    return *this;
  }
};
  ...
  : zero_wrapper_impl<TExpr, function_traits_t<decltype(&TExpr::operator())> >
{
  ...
  const TExpr& get() const
  {
    return reinterpret_cast<const TExpr&>(*this);
  }
};
namespace detail
{
...
auto
get_type_name(const char* ptr, index_sequence<Ns...>)
{
...
{
#if defined(_MSC_VER)
  return detail::get_type_name<T, 34>(
    __FUNCSIG__, make_index_sequence<sizeof(__FUNCSIG__) - 34 - 8>{});
#elif defined(__clang__)
  return detail::get_type_name<T, 58>(
    __PRETTY_FUNCTION__,
    make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 58 - 2>{});
#elif defined(__GNUC__)
...
struct on_entry : internal_event, entry_exit
{
  ...
  explicit on_entry(const TEvent& event = {})
    : event_(event)
  {
  }
  const TEvent& event_;
...
struct on_exit : internal_event, entry_exit
{
  ...
  explicit on_exit(const TEvent& event = {})
    : event_(event)
  {
  }
  const TEvent& event_;
...
template <class... Ts>
using get_all_events = aux::join_t<typename get_all_events_impl<
  typename Ts::src_state, typename Ts::dst_state, typename Ts::event>::type...>;
template <class... Ts>
...
template <class... Ts>
using get_initial_states = aux::join_t<
  typename aux::conditional<Ts::initial, aux::type_list<typename Ts::src_state>,
                            aux::type_list<> >::type...>;
template <class... Ts>
using get_history_states =
  aux::join_t<typename aux::conditional<!Ts::history && Ts::initial,
                                        aux::type_list<typename Ts::src_state>,
                                        aux::type_list<> >::type...>;
template <class>
...
struct sub_sm<sm_impl<sm_policy<T> > >
{
  ...
  static sm_impl<sm_policy<T, TPolicies...> >& get(
  ...
  template <class... TPolicies>
  static const sm_impl<sm_policy<T, TPolicies...> >& cget(
...
{
  using sm                   = SM;
  using thread_safety_policy = decltype(
    get_policy<thread_safety_policy__>((aux::inherit<TPolicies...>*)0));
  using defer_queue_policy =
    decltype(get_policy<defer_queue_policy__>((aux::inherit<TPolicies...>*)0));
  using logger_policy =
    decltype(get_policy<logger_policy__>((aux::inherit<TPolicies...>*)0));
  using testing_policy =
    decltype(get_policy<testing_policy__>((aux::inherit<TPolicies...>*)0));
  template <class T>
...
void
log_process_event(const aux::type<no_policy>&, TDeps&, const TEvent&)
{
}
template <class SM, class TLogger, class TDeps, class TEvent>
void
log_process_event(const aux::type<TLogger>&, TDeps& deps, const TEvent& event)
{
...
void
log_state_change(const aux::type<no_policy>&, TDeps&, const TSrcState&,
                 const TDstState&)
{
}
template <class SM, class TLogger, class TDeps, class TSrcState,
          class TDstState>
void
log_state_change(const aux::type<TLogger>&, TDeps& deps, const TSrcState&,
                 const TDstState&)
{
  return static_cast<aux::pool_type<TLogger&>&>(deps)
...
void
log_action(const aux::type<no_policy>&, TDeps&, const TAction&, const TEvent&)
{
}
template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
void
log_action(const aux::type<TLogger>&, TDeps& deps, const TAction& action,
           const TEvent& event)
{
  return static_cast<aux::pool_type<TLogger&>&>(deps)
...
template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
void
log_action(const aux::type<TLogger>&, TDeps& deps,
           const aux::zero_wrapper<TAction>& action, const TEvent& event)
{
  return static_cast<aux::pool_type<TLogger&>&>(deps)
...
void
log_guard(const aux::type<no_policy>&, TDeps&, const TGuard&, const TEvent&,
          bool)
{
}
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
void
log_guard(const aux::type<TLogger>&, TDeps& deps, const TGuard& guard,
...
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
void
log_guard(const aux::type<TLogger>&, TDeps& deps,
          const aux::zero_wrapper<TGuard>& guard, const TEvent& event,
          bool result)
{
  return static_cast<aux::pool_type<TLogger&>&>(deps)
...
struct transitions<T>
{
  ...
  template <class TEvent, class SM, class TDeps, class TSubs>
  static bool execute_impl(const TEvent& event, SM& sm, TDeps& deps,
                           TSubs& subs, typename SM::state_t& current_state)
  {
  ...
  static bool execute_impl(const on_exit<_, TEvent>& event, SM& sm, TDeps& deps,
                           TSubs& subs, typename SM::state_t& current_state)
  {
    aux::get<T>(sm.transitions_)
      .execute(event, sm, deps, subs, current_state,
               typename SM::has_entry_exits{});
    return false;
  }
...
struct transitions_sub<sm<TSM>, T, Ts...>
{
  ...
  static bool execute(const anonymous& event, SM& sm, TDeps& deps, TSubs& subs,
                      typename SM::state_t& current_state)
  {
    if (sub_sm<sm_impl<TSM> >::cget(&subs).is_terminated())
    {
      ...
      return handled ? handled : transitions<T, Ts...>::execute(
  ...
  template <class _, class TEvent, class SM, class TDeps, class TSubs>
  static bool execute_impl(const back::on_entry<_, TEvent>& event, SM& sm,
...
{
  template <class TEvent, class SM, class TDeps, class TSubs>
  static bool execute(const TEvent& event, SM&, TDeps& deps, TSubs& subs,
                      typename SM::state_t&)
  {
    sub_sm<sm_impl<TSM> >::get(&subs).template process_event<TEvent>(
      event, deps, subs);
    return true;
  }
};
...
struct get_mapping<event<E>, event<E>, T, R>
  : aux::type_list<event_mappings<
      E, aux::apply_t<unique_mappings_t,
...
struct get_mapping<state<S>, state<S>, T, R>
  : aux::type_list<
      state_mappings<S, aux::join_t<typename R::types, typename T::types> > >
{
};
template <class T, class... Ts>
struct extend_mapping
  : aux::join_t<typename get_mapping<typename T::element_type,
...
struct mappings<aux::pool<Ts...> >
  : unique_mappings_t<event_mappings<
      typename Ts::event, aux::inherit<state_mappings<
                            typename Ts::src_state, aux::type_list<Ts> > > >...>
{
};
template <class T>
...
struct callable
  : decltype(test_callable<
             aux::inherit<aux::conditional_t<__is_class(T), T, aux::none_type>,
                          callable_fallback> >(0))
{
};
}
#if !defined(BOOST_SML_DISABLE_EXCEPTIONS)
#define BOOST_SML_DISABLE_EXCEPTIONS                                           \
...
struct sm_impl
{
  ...
  using events_t =
    aux::apply_t<aux::unique_t,
                 aux::join_t<sub_internal_events_t,
                             aux::apply_t<get_events, transitions_t> > >;
  ...
  sm_impl(const aux::init&, const TPool& p)
    : sm_impl(p, decltype(aux::has_type<sm_t&>(&p)){})
  {
  }
  template <class TPool>
  sm_impl(const TPool& p, aux::true_type)
    : transitions_(aux::cget<sm_t&>(p)())
    , sm_()
  {
    initialize(typename sm_impl<TSM>::initial_states_t{});
  }
  template <class TPool>
  sm_impl(const TPool&, aux::false_type)
    : sm_()
    , transitions_(sm_())
  {
    initialize(typename sm_impl<TSM>::initial_states_t{});
...
#if BOOST_SML_DISABLE_EXCEPTIONS
    const auto handled =
      process_event_impl<get_event_mapping_t<get_generic_t<TEvent>, mappings> >(
        event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
    const auto handled = process_event_noexcept<
      get_event_mapping_t<get_generic_t<TEvent>, mappings> >(event, deps, subs,
                                                             has_exceptions{});
#endif
    process_internal_events(anonymous{}, deps, subs);
    process_defer_events(deps, subs, handled,
                          ...
                          aux::index_sequence<Ns...>)
  {
    ...
    const auto lock    = create_lock(aux::type<thread_safety_t>{});
    (void)lock;
    (void)aux::swallow{ 0, (handled |= dispatch_table[current_state_[Ns]](
                              event, *this, deps, subs, current_state_[Ns]),
                            0)... };
    return handled;
  }
  template <class TMappings, class TEvent, class TDeps, class TSubs,
                          ...
                          state_t& current_state)
  {
    ...
    const auto lock = create_lock(aux::type<thread_safety_t>{});
    (void)lock;
    return dispatch_table[current_state](event, *this, deps, subs,
                                         current_state);
  }
#if !BOOST_SML_DISABLE_EXCEPTIONS
  template <class TMappings, class TEvent, class TDeps, class TSubs>
  bool process_event_noexcept(const TEvent& event, TDeps& deps, TSubs& subs,
                              aux::false_type) noexcept
  {
  ...
  template <class TMappings, class TEvent, class TDeps, class TSubs>
  bool process_event_noexcept(const TEvent& event, TDeps& deps, TSubs& subs,
                              state_t& current_state, aux::false_type) noexcept
  {
    return process_event_impl<TMappings>(event, deps, subs, states_t{},
                                         current_state);
  }
  template <class TMappings, class TEvent, class TDeps, class TSubs>
  bool process_event_noexcept(const TEvent& event, TDeps& deps, TSubs& subs,
                              state_t& current_state, aux::true_type) noexcept
  {
    try
    {
      return process_event_impl<TMappings>(event, deps, subs, states_t{},
  ...
  void process_defer_events(TDeps&, TSubs&, const bool,
...
#if BOOST_SML_DISABLE_EXCEPTIONS
    const auto handled =
      process_event_impl<get_event_mapping_t<TEvent, mappings> >(
        event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
    const auto handled =
      process_event_noexcept<get_event_mapping_t<TEvent, mappings> >(
  ...
  void process_defer_events(TDeps& deps, TSubs& subs, const bool handled,
                            ...
                            const aux::type_list<TEvents...>&)
  {
    if (handled)
    {
      ...
      while (size-- && (this->*dispatch_table[defer_.front().id])(
                         deps, subs, defer_.front().data))
        ;
    }
  }
  template <class TVisitor, class... TStates>
  void visit_current_states(const TVisitor& visitor,
                            const aux::type_list<TStates...>&,
                            ...
                            aux::index_sequence<Ns...>) const
  {
    ...
    (void)aux::swallow{ 0,
                        (dispatch_table[current_state_[Ns]](visitor), 0)... };
  }
  ...
  no_policy create_lock(const aux::type<no_policy>&)
  {
    return {};
  }
  ...
  auto create_lock(const aux::type<TLockPolicy>&)
  {
    struct lock_guard
    {
      explicit lock_guard(thread_safety_t& synch)
        : thread_safety_(synch)
      {
        thread_safety_.lock();
      }
      ~lock_guard()
  ...
  {
    auto result = true;
    (void)aux::swallow{ 0, (current_state_[Ns] ==
                                aux::get_id<state_t, terminate_state>(
                                  (states_ids_t*)0)
                              ? result
                              : (result = false))... };
    return result;
...
private:
  using sm_all_t =
    aux::apply_t<aux::inherit,
                 aux::join_t<aux::type_list<sm_t>,
                             aux::apply_t<get_sm_t, state_machines> > >;
  using sub_sms_t = aux::apply_t<
    aux::pool,
    typename convert_to_sm<TSM, aux::apply_t<get_sub_sms, states> >::type>;
  using deps   = aux::apply_t<merge_deps, transitions_t>;
  using deps_t = aux::apply_t<
    aux::pool,
    aux::apply_t<
      aux::unique_t,
      aux::join_t<deps, logger_dep_t, aux::apply_t<merge_deps, sub_sms_t> > > >;
...
public:
  ...
  sm(aux::init, deps_t& deps, typename TSM::sm& sm)
    : deps_(deps)
    , sub_sms_{ aux::pool<typename TSM::sm&, deps_t>{ sm, deps } }
  {
    aux::get<sm_impl<TSM> >(sub_sms_).start(deps_, sub_sms_);
  }
  template <class... TDeps,
            __BOOST_SML_REQUIRES(aux::is_unique_t<TDeps...>::value)>
  sm(TDeps&&... deps)
  ...
  void process_event(const TEvent& event)
  {
    aux::get<sm_impl<TSM> >(sub_sms_).process_event(
  ...
  bool is(const TState&) const
  {
    ...
    visit_current_states<T>([&](auto state) {
      (void)state;
      result |= aux::is_same<typename TState::type,
                             typename decltype(state)::type>::value;
    });
    return result;
  }
  template <
  ...
  bool is(const TStates&...) const
  {
    ...
    state_t state_ids[] = { aux::get_id<state_t, typename TStates::type>(
      (states_ids_t*)0)... };
    visit_current_states<T>([&](auto state) {
      (void)state;
  ...
  void __set_current_states(const TStates&...)
  {
    ...
    auto  region       = 0;
    (void)aux::swallow{ 0, (sm.current_state_[region++] =
                              aux::get_id<state_t, typename TStates::type>(
                                (states_ids_t*)0),
                            0)... };
  }
  ...
  auto& getInfo()
  {
    return aux::get<sm_impl<TSM> >(sub_sms_).sm();
  }
...
template <class T, class E, class>
auto args__(int) -> aux::function_traits_t<decltype(
  &T::__BOOST_SML_TEMPLATE_KEYWORD operator() < back::get_event_t<E>>)>;
template <class T, class E, class TSM>
auto args__(int) -> aux::function_traits_t<decltype(
  &T::__BOOST_SML_TEMPLATE_KEYWORD operator() < back::get_event_t<E>, TSM >)>;
template <class T, class E, class TSM>
...
decltype(auto)
  get_arg(const aux::type<sm_ref_fwd&>&, const TEvent&, TSM& sm, TDeps&)
{
  return sm;
}
template <class TEvent, class TSM, class TDeps>
decltype(auto)
  get_arg(const aux::type<TEvent>&, const TEvent& event, TSM&, TDeps&)
{
  return event;
}
template <class TEvent, class TSM, class TDeps>
decltype(auto)
  get_arg(const aux::type<const TEvent&>&, const TEvent& event, TSM&, TDeps&)
{
  return event;
}
template <class T, class TEvent, class TSM, class TDeps>
decltype(auto)
  get_arg(const aux::type<const TEvent&>&,
          const back::unexpected_event<T, TEvent>& event, TSM&, TDeps&)
{
  return event.event_;
}
template <class T, class TEvent, class TSM, class TDeps>
decltype(auto) get_arg(const aux::type<const TEvent&>&,
                       const back::on_entry<T, TEvent>& event, TSM&, TDeps&)
{
  return event.event_;
}
template <class T, class TEvent, class TSM, class TDeps>
decltype(auto) get_arg(const aux::type<const TEvent&>&,
                       const back::on_exit<T, TEvent>& event, TSM&, TDeps&)
{
  return event.event_;
}
template <class T, class TEvent, class TSM, class TDeps>
decltype(auto) get_arg(const aux::type<T>&,
                       const back::exception<TEvent>& event, TSM&, TDeps&)
{
...
struct call<TEvent, aux::type_list<>, TLogger>
{
  ...
  static auto execute_impl(const aux::type<bool>&, T object,
  ...
  template <class TSM, class T, class TDeps>
  static auto execute_impl(const aux::type<void>&, T object,
                           const TEvent& event, TDeps& deps)
  {
...
struct call<TEvent, aux::type_list<Ts...>, TLogger>
{
                      ...
                      TSubs& subs)
  {
    sm_ref<TSM, TDeps, TSubs> sm_ref{ sm, deps, subs };
    using result_type =
      decltype(object(get_arg(aux::type<Ts>{}, event, sm_ref, deps)...));
    return execute_impl<typename TSM::sm_t>(aux::type<result_type>{}, object,
                                            event, sm, deps, subs);
  }
...
private:
  ...
  void for_all(const aux::index_sequence<Ns...>&, const TEvent& event, TSM& sm,
               TDeps& deps, TSubs& subs)
  {
    (void)aux::swallow{
...
private:
  template <int... Ns, class TEvent, class TSM, class TDeps, class TSubs>
  auto for_all(const aux::index_sequence<Ns...>&, const TEvent& event, TSM& sm,
               TDeps& deps, TSubs& subs)
  {
    auto result = true;
    (void)aux::swallow{
...
{
template <class T,
          __BOOST_SML_REQUIRES(concepts::composable<typename T::sm>::value)>
...
template <class T, class TPolicy, class... TPolicies>
using rebind_policies_t =
  typename rebind_policies<T, TPolicy, TPolicies...>::type;
}
class SM;
template <class T = SM, class TPolicies = Policies<>, class... Ts>
struct sm
  : detail::state_machine<detail::rebind_policies_t<T, TPolicies, Ts...> >
{
  using detail::state_machine<
    detail::rebind_policies_t<T, TPolicies, Ts...> >::state_machine;
};
namespace concepts
{
...
struct process
{
  ...
  {
  public:
    explicit process_impl(const TEvent& event)
      : event(event)
    {
    }
    template <class T, class TSM, class TDeps, class TSubs>
...
struct event
{
  ...
  {
    return transition_eg<event, aux::zero_wrapper<T> >{
  ...
  auto operator/(const T& t) const
  {
    return transition_ea<event, aux::zero_wrapper<T> >{
      *this, aux::zero_wrapper<T>{ t }
    };
  }
  auto operator()() const
...
struct ignore<E, aux::type_list<Ts...> >
{
  ...
  struct non_events
  {
    using type = aux::conditional_t<
      aux::is_same<back::get_event_t<E>, aux::remove_reference_t<T> >::value ||
...
struct transition<state<S2>, G, A>
  : transition<state<internal>, state<S2>, front::event<back::anonymous>, G, A>
{
  ...
  {
    return transition<T, state<S2>, front::event<back::anonymous>, G, A>{ g,
                                                                          a };
  }
};
  ...
  : transition<state<S1>, state<S2>, front::event<E>, always, none>
{
  transition(const state<S1>&, const transition<state<S2>, front::event<E> >&)
    : transition<state<S1>, state<S2>, front::event<E>, always, none>{ always{},
                                                                       none{} }
  {
  }
};
  ...
  : transition<state<internal>, state<S2>, front::event<E>, G, none>
{
  ...
  transition(const state<S2>&, const transition_eg<front::event<E>, G>& t)
    : transition<state<internal>, state<S2>, front::event<E>, G, none>{ t.g,
                                                                        none{} }
  {
  }
  template <class T>
...
void
update_composite_states(TSubs& subs, aux::true_type,
                        const aux::type_list<THs...>&)
{
  using state_t = typename T::state_t;
  auto& sm      = back::sub_sm<T>::get(&subs);
  (void)sm;
  (void)aux::swallow{ 0, (sm.current_state_[aux::get_id<state_t, THs>(
                            (typename T::initial_states_ids_t*)0)] =
                            aux::get_id<state_t, THs>(
                              (typename T::states_ids_t*)0),
                          0)... };
}
template <class T, class TSubs, class... Ts>
void
update_composite_states(TSubs& subs, aux::false_type, Ts&&...)
{
...
void
update_current_state(SM&, TDeps& deps, TSubs&,
                     typename SM::state_t&       current_state,
                     const typename SM::state_t& new_state, const TSrcState&,
                     const TDstState&)
{
  back::log_state_change<typename SM::sm_t>(
...
void
update_current_state(SM&, TDeps& deps, TSubs& subs,
                     typename SM::state_t&       current_state,
                     const typename SM::state_t& new_state, const TSrcState&,
...
struct transition<state<S1>, state<S2>, front::event<E>, G, A>
{
  ...
  template <class TEvent, class SM, class TDeps, class TSubs>
  bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
...
struct transition<state<S1>, state<S2>, front::event<E>, always, A>
{
  ...
  transition(const always&, const A& a)
    : a(a)
  {
  }
  template <class TEvent, class SM, class TDeps, class TSubs>
...
struct transition<state<S1>, state<S2>, front::event<E>, G, none>
{
  ...
  transition(const G& g, const none&)
    : g(g)
  {
  }
  template <class TEvent, class SM, class TDeps, class TSubs>
...
struct transition<state<S1>, state<S2>, front::event<E>, always, none>
{
  ...
  template <class TEvent, class SM, class TDeps, class TSubs>
  bool execute(const TEvent&, SM& sm, TDeps& deps, TSubs& subs,
               typename SM::state_t& current_state, aux::false_type)
  {
    update_current_state(sm, deps, subs, current_state,
                         aux::get_id<typename SM::state_t, dst_state>(
